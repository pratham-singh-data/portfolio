...
export const ProjectsPage = () => {
  ...
  useEffect(() => {
    if (!filter) {
      // display all data if no filters are set
      setSamples(fullData);
      return;
    }

    setSamples(
      fullData.filter((entry) => entry.categories.indexOf(filter) !== -1)
    );
  }, [filter, fullData]);
  ...
  return (
    <Box>
      ...
      >
        <Typography sx={{ color: "#272727", display: "inline" }} variant="h5">
          Results:
        </Typography>
        <TextField
          sx={{
            width: { md: "full", xs: "100%" },
            mx: { md: "50px", xs: "0px" },
          }}
          placeholder="Search here..."
          value={searchTerm}
          onChange={onSearchEnterred}
          type="search"
        />
      </Box>
